...
\begin{itemize}
  ...
  \item \textbf{Separazione}: calcolo e I/O (`print`) sono distinti per chiarezza.
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
  ...
  \item \textbf{Ordine}: dal 3.7 l'ordine d'inserimento è preservato; non confonderlo con ordinamento.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Spiegazione del codice]
\begin{itemize}
  \item \textbf{Unione/intersezione/differenza}: \texttt{|}, \texttt{\&}, \texttt{-} operano su insiemi con semantica matematica.
  \item \textbf{Unicità}: i set eliminano duplicati; elementi devono essere hashable.
  \item \textbf{Prestazioni}: membership e operazioni insiemistiche sono mediamente O(1)/O(n).
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
  \item Modellare dati di dominio (liste, dizionari, set).
  \item Cache LRU e code con \texttt{deque}.
  \item Rilevare duplicati e membership con \texttt{set}.
  \item Mappare configurazioni e lookup con \texttt{dict}.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Spiegazione del codice]
\begin{itemize}
  \item \textbf{Counter}: conteggio frequenze e \texttt{most\_common} per ranking rapido.
  \item \textbf{Comprehension dict}: crea indice per chiave evitando loop manuali.
  \item \textbf{defaultdict}: inizializza accumulatori riducendo boilerplate.
  \item \textbf{sorted key}: ordinamento stabile su tuple di chiavi senza mutare l'input.
\end{itemize}
\end{tcolorbox}
\end{tcolorbox}
