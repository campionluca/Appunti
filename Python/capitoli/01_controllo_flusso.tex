...
\begin{itemize}
  \item \textbf{Branching}: catene `if/elif/else` gestiscono casi esclusivi.
  \item \textbf{Confronto}: `==` vs `<`; preferire condizioni semplici e nomi espliciti.
  \item \textbf{I/O}: la stampa è l'effetto; separare logica dal lato utente quando possibile.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Spiegazione del codice]
\begin{itemize}
  \item \textbf{range}: genera 0..2; preferire `enumerate(seq)` quando si itera su collezioni.
  \item \textbf{while}: usa condizione booleana e aggiorna stato (`count += 1`).
  \item \textbf{Chiarezza}: evitare `while True` con break se un range o un iterabile è sufficiente.
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
  \item Orchestrare logiche di business e validazioni.
  \item Implementare state machine e menu interattivi.
  \item Gestire retry/backoff e circuit breaker semplificati.
  \item Costruire pipeline di elaborazione condizionale.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Spiegazione del codice]
\begin{itemize}
  \item \textbf{Indice vs elemento}: il primo snippet mostra stile basato su indice; in Python si preferisce iterare su elementi (`for x in arr`).
  \item \textbf{Filtri}: usare `continue` per saltare i pari; l'alternativa compatta usa generator expression in `sum(...)`.
  \item \textbf{for-else}: il blocco `else` si esegue solo se il ciclo non ha usato `break` (elemento non trovato); utile per ricerca con fallback.
  \item \textbf{Stato}: `total` accumula; separare accumulo dalla logica di decisione semplifica i test.
\end{itemize}
\end{tcolorbox}
\end{tcolorbox}
