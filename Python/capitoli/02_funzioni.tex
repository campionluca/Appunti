...
\begin{itemize}
  ...
  \item \textbf{*args}: raccoglie posizione variabile di argomenti in una tupla.
  \item \textbf{**kwargs}: raccoglie coppie chiave/valore in un dizionario; utile per parametri nominati flessibili.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Spiegazione del codice]
\begin{itemize}
  \item \textbf{Scope}: `mul` legge `factor` dallo scope esterno (E nel LEGB).
...
\section{Esercizi}
\label{sec:fun-esercizi}
\begin{enumerate}
  \item Scrivi una funzione che calcola il fattoriale di un numero (iterativa e ricorsiva).
  \item Crea una funzione \texttt{safeDiv(a, b)} che gestisce divisione per zero.
...
\label{sec:fun-contesto}
\begin{tcolorbox}[title=Contesto e Applicazioni]
\begin{itemize}
  \item Progettare API di modulo e funzioni riutilizzabili.
  \item Funzioni pure per testabilità e caching.
  \item Higher-order functions per composizione.
  \item Validazione e parsing input incapsulati.
\end{itemize}
\end{tcolorbox}

\section{Approfondimenti}
\label{sec:fun-approfondimenti}
...
def appendItemBad(item, bucket=[]):
    bucket.append(item)
    return bucket

# Uso corretto
def appendItem(item, bucket=None):
    if bucket is None:
        bucket = []
    bucket.append(item)
    return bucket
...
\begin{tcolorbox}[title=Spiegazione del codice]
\begin{itemize}
  \item \textbf{Default mutabile}: `bucket=[]` condivide lista tra chiamate; bug classico.
  \item \textbf{Pattern corretto}: usa `None` e inizializza all'interno per isolare stato.
  \item \textbf{Type hints}: `Iterable[int]` e `Callable` comunicano contratto; non impongono a runtime.
  \item \textbf{Keyword args}: chiamata `transform(xs=data, fn=...)` aumenta chiarezza dei parametri.
\end{itemize}
\end{tcolorbox}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Spiegazione del codice]
\begin{itemize}
  \item \textbf{Docstring}: descrive contratto e output atteso.
  \item \textbf{max or 1}: fallback per evitare divisione per zero su lista vuota.
  \item \textbf{Comprensione}: crea nuova lista normalizzata; `pipeline` mostra composizione filtrando positivi.
\end{itemize}
\end{tcolorbox}
\end{tcolorbox}
