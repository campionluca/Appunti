...
\begin{itemize}
  \item \textbf{Qualità}: Catturare bug prima della produzione
  \item \textbf{Refactoring sicuro}: Modificare codice con confidenza
  \item \textbf{Documentazione}: Test come esempi d'uso
  \item \textbf{Design}: Test influenzano architettura (testabile = modulare)
  \item \textbf{Regressioni}: Prevenire re-introduzione di bug
\end{itemize}
...
\textbf{Unit Tests} (70\%):
\begin{itemize}
  \item Testano singola funzione/classe isolatamente
  \item Veloci (ms), stabili, facili da debuggare
  \item Mock dependencies esterne
\end{itemize}

\textbf{Integration Tests} (20\%):
\begin{itemize}
  \item Testano interazione tra componenti
  \item Database, API, file system reali
  \item Più lenti, più setup richiesto
\end{itemize}

\textbf{End-to-End Tests} (10\%):
\begin{itemize}
  \item Testano applicazione completa (UI, API, DB)
  \item Molto lenti, fragili (flaky)
  \item Solo per flussi critici
\end{itemize}
