...
\begin{itemize}
  \item Il decoratore \texttt{logCalls} avvolge la funzione per eseguire codice prima/dopo.
  \item Usare \verb|functools.wraps| su \texttt{wrapper} per preservare metadati (nome, docstring).
  \item I decoratori sono ottimi per logging, validazione, caching e autorizzazioni.
\end{itemize}
\end{tcolorbox}
...
\begin{tikzpicture}[
    ...
    \node[deco] (def_deco) at (0,6) {\texttt{def logCalls(fn):}};
    ...
    \node[func] (def_func) at (4.5,5) {@logCalls\\def hello():};
    ...
    % Output
    \node[box, fill=yellow!20, text width=4cm] (output) at (2.5,0) {
        \textbf{Output:}\\
...
\begin{tikzpicture}[
    ...
    \node[font=\footnotesize, align=left] at (4.5,4) {Livello 1: Parametri decorator};
    \node[font=\footnotesize, align=left] at (4.5,3) {Livello 2: Decorator vero};
    \node[font=\footnotesize, align=left] at (4.5,2) {Livello 3: Wrapper};
    \node[font=\footnotesize, align=left] at (4.5,1) {Funzione originale};
\end{tikzpicture}
\end{center}
...
\begin{tcolorbox}[title=Spiegazione]
\begin{itemize}
  \item Implementa il protocollo iteratore con \verb|__iter__| e \verb|__next__|.
  \item Solleva \texttt{StopIteration} per segnalare fine; consuma elemento per chiamata.
  \item Preferire iteratori/generatori per flussi potenzialmente grandi o infiniti.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Spiegazione]
\begin{itemize}
  \item I generatori producono valori lazily con \texttt{yield}; consumati da \texttt{for} o \texttt{list(...)}.
  \item Consentono pipeline efficienti e gestione di risorse con \verb|try/finally|.
  \item Comporre trasformazioni con generator expression e moduli come \texttt{itertools}.
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[title=Casi d'uso]
\begin{itemize}
  \item Logging/monitoring trasparente con decoratori.
  \item Streaming di file/HTTP con trasformazioni lazy.
  \item Validazione/autorità dei parametri senza cambiare funzioni originali.
  \item Rate limiting e retry come decoratori riutilizzabili.
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
  \item Strumentazione trasparente (logging, timing) con decoratori.
  \item Pipeline dati lazy e streaming efficiente.
  \item Validazioni, autorizzazioni e caching via decoratori.
  \item Generator expression per filtri e trasformazioni.
\end{itemize}
\end{tcolorbox}
