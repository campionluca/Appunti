...
Questa situazione rivela una serie di problemi evidenti. Innanzitutto, è praticamente impossibile sapere cosa sia effettivamente cambiato tra una versione e l'altra, poiché i file hanno nomi simili senza informazioni sui dettagli delle modifiche. Non esiste alcuna traccia sistematica di chi ha fatto le modifiche, quando e per quale motivo. Collaborare diventa estremamente difficile: con múltiple "versioni finali" in circolazione, rimane sempre il dubbio su quale sia effettivamente la versione corretta da usare. Se qualcosa va storto, non esiste un modo sistematico di tornare indietro facilmente a uno stato precedente noto come stabile. Infine, i nomi dei file diventano rapidamente un caos di suffissi e numeri (_v1, _v2, _final, _REALMENTE_FINALE) che non seguono alcun criterio logico.
...
Un Version Control System come Git risolve elegantemente tutti questi problemi. Innanzitutto, offre un tracking automatico di ogni modifica al codice, con informazioni dettagliate su timestamp e autore. Ogni cambio viene registrato permanentemente, creando una cronologia immodificabile.

Accompagnando ogni insieme di modifiche, è possibile aggiungere commit messages descrittivi che spiegano non solo cosa è stato modificato, ma soprattutto il perché, fornendo contesto essenziale per la comprensione futura del codice.

Git consente il branching, ovvero la capacità di creare linee di sviluppo parallele e isolate dove lavorare su feature diverse senza che i cambiamenti interferi scano gli uni con gli altri. Questa è una delle caratteristiche più potenti di Git.

Quando necessario, il rollback consente di tornare a versioni precedenti in modo istantaneo, sia che si tratti di revertare un singolo file o ripristinare l'intero progetto a uno stato precedente.

La collaborazione diventa naturale: múltiple persone possono lavorare sugli stessi file contemporaneamente in branch diversi, e Git facilita l'integrazione intelligente delle loro modifiche.

Infine, Git fornisce un audit trail completo del progetto: è sempre possibile visualizzare l'intera storia delle modifiche, comprendere quando è stato introdotto un bug, chi l'ha introdotto e come è stato fixato.
...
In un'architettura CVCS, un server centrale è l'unico detentore della storia completa del progetto, mentre i client hanno accesso solo alla versione più recente dei file (la cosiddetta working copy). Ogni operazione, dal commit al visualizzare la storia, richiede una connessione al server. Il flusso è semplice: quando effettui un commit locale, questo viene immediatamente pushato al server centrale.

Questo modello ha alcuni vantaggi. Consente un'amministrazione centralizzata che facilita il controllo degli accessi e la gestione delle autorizzazioni. Il modello mentale è relativamente semplice: esiste un punto di verità centrale. Inoltre, il backup è naturalmente centralizzato sul server.

Tuttavia, gli svantaggi sono significativi. Il server rappresenta un single point of failure: se il server non è disponibile, nessuno può fare commit, visualizzare la storia o lavorare con il version control. Poiché ogni operazione richiede connessione di rete, è impossibile fare commit offline o lavorare quando la connessione è instabile. Le operazioni come visualizzare le differenze tra versioni o consultare la storia richiedono query al server, rendendole lente rispetto a operazioni locali. Creare un branch è un'operazione pesante che comporta duplicazione di file sul server. Infine, esiste il rischio di perdita di dati catastrofica: se il server si arresta in modo imprevisto, la storia del progetto potrebbe andare persa.
...
Un DVCS ha un'architettura radicalmente diversa. Ogni clone di un repository è una copia completa del repository stesso, includendo l'intera cronologia. Questo significa che ogni sviluppatore ha localmente accesso a tutta la storia del progetto. Le operazioni locali come commit, creazione di branch e merge sono istantanee poiché operano su file locali senza accesso alla rete. La sincronizzazione con altri repository è esplicita: gli sviluppatori decidono consapevolmente quando fare push (inviare i loro commit) o pull (ricevere i commit degli altri). Grazie a questa architettura, è possibile lavorare completamente offline, fare commit e creare branch anche senza connessione internet.

I vantaggi sono sostanziali. Innanzitutto, esiste un backup naturale: ogni clone è un backup completo dell'intero progetto, quindi la perdita di dati è pressoché impossibile. Le operazioni sono incredibilmente veloci perché tutte le operazioni locali avvengono senza latenza di rete. Lo stile di lavoro offline-first consente sviluppo completo anche senza accesso a internet. Creare branch è istantaneo perché implica semplicemente la creazione di un puntatore, senza duplicazione di file. I DVCS supportano workflow complessi e flessibili, adattabili a diverse necessità di sviluppo. Infine, un sistema di checksum garantisce che non possa avvenire corruzione di dati.

Lo svantaggio principale dei DVCS è una curva di apprendimento più ripida. Richiedono una comprensione di concetti più sofisticati come la staging area, il rebase e la sincronizzazione esplicita. Inoltre, lo storage locale è maggiore poiché ogni sviluppatore mantiene una copia completa della storia.
...
L'approccio basato su snapshot offre numerosi vantaggi pratici. Il checkout diventa velocissimo perché Git semplicemente copia i file dalla versione richiesta, senza dover ricostruire lo stato applicando una sequenza di delta come fanno altri VCS. Il branching risulta istantaneo poiché creare un nuovo branch significa semplicemente creare un nuovo puntatore, senza duplicazione di file. Infine, l'integrità è garantita dal checksum SHA-1 applicato a ogni oggetto, assicurando che qualsiasi corruzione o modifica non autorizzata venga immediatamente rilevata.
...
Il sistema di checksum SHA-1 possiede proprietà fondamentali che garantiscono l'affidabilità di Git. L'hash è univoco: la probabilità di collisione tra due oggetti diversi è praticamente zero, rendendo ogni hash un identificatore sicuro. Il sistema è immutabile: modificare anche un solo byte del contenuto produce un hash completamente diverso, rendendo impossibile alterare dati senza che Git lo rilevi. Questa verificabilità automatica significa che qualsiasi corruzione dei dati viene immediatamente identificata quando Git calcola e confronta i checksum.
