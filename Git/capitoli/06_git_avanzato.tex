...
Usa cherry-pick con cautela poiché crea duplicazione nella cronologia e può causare confusione. Quando possibile, preferisci merge. Detto questo, cherry-pick è ottimo per scenari specifici: hotfix urgenti che devono essere applicati a múltiple branch, recupero di commit da branch che verranno cancellati, e applicazione di commit specifici senza dover mergeare l'intero branch.
\end{tcolorbox}
...
Reset soft è perfetto per combinare múltiple commit in uno unico, per riscrivere i messaggi di múltiple commit insieme, o per riorganizzare modifiche che sono già state committate.
\end{tcolorbox}
...
Reflog è incredibilmente utile per recuperare da errori: conserva la cronologia per 90 giorni per default e può salvare il tuo lavoro da reset disastrosi. Tuttavia, ci sono alcune limitazioni da ricordare. Reflog è locale e non viene pushato ad altri repository, quindi il recovery è possibile solo localmente. Dopo 90 giorni, le entry vengono rimosse automaticamente. Inoltre, \texttt{git gc} può pulire oggetti non referenziati anche prima di questo periodo.
\end{tcolorbox}
...
Lo Semantic Versioning segue uno schema di versioning \texttt{MAJOR.MINOR.PATCH}. La componente MAJOR aumenta quando vengono introdotti cambiamenti incompatibili (breaking changes) che richiedono agli utenti di adattare il loro codice. La componente MINOR aumenta quando vengono aggiunte nuove funzionalità in modo compatibile con versioni precedenti. La componente PATCH aumenta quando vengono rilasciati bugfix che non cambiano l'API.

Per esempio, la versione \texttt{v1.0.0} rappresenta la prima versione stabile. La versione \texttt{v1.1.0} contiene nuove feature aggiunte in modo compatibile con la 1.0.0. La versione \texttt{v1.1.1} contiene bugfix minori della 1.1.0. La versione \texttt{v2.0.0} contiene cambiamenti incompatibili e non è compatibile con la versione 1.x.x.
\end{tcolorbox}
