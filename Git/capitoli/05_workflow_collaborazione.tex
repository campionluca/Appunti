...
\begin{itemize}
    \item Sviluppatori committano direttamente su \texttt{main}
    \item Produzione rompe frequentemente
    \item Feature incomplete vanno in produzione
    \item Impossibile rollback selettivo
    \item Hotfix mescolati con feature
    \item Code review inconsistente
    \item Chaos totale in team \textgreater 2 persone
\end{itemize}
...
\begin{itemize}
    \item \textbf{Separazione}: Feature/hotfix/release isolati
    \item \textbf{Stabilità}: \texttt{main} sempre deployabile
    \item \textbf{Qualità}: Code review obbligatoria
    \item \textbf{Tracciabilità}: Ogni feature ha branch dedicato
    \item \textbf{Release control}: Gestione versioni chiara
    \item \textbf{Rollback}: Facile tornare indietro
\end{itemize}
...
\begin{itemize}
    \item Struttura chiara e ben documentata
    \item Adatto a release pianificate (ogni mese, trimestre)
    \item Separazione netta develop/produzione
    \item Hotfix non bloccano sviluppo feature
    \item Supporta release multiple in parallelo
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!10, colframe=red!60, title=Svantaggi Git Flow]
\begin{itemize}
    \item Complesso per team piccoli o progetti semplici
    \item Overhead di gestione branch
    \item Non adatto a continuous deployment
    \item Curva apprendimento per junior
    \item Merge conflicts frequenti con branch longevi
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
    \item Semplicissimo: un branch principale + feature
    \item Ideale per continuous deployment
    \item Code review integrata (Pull Request)
    \item Deploy frequenti (anche più volte al giorno)
    \item Branch short-lived (meno conflitti)
    \item Curva apprendimento bassa
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!10, colframe=red!60, title=Svantaggi GitHub Flow]
\begin{itemize}
    \item Non adatto a release pianificate
    \item Richiede CI/CD robusto
    \item Richiede testing automatico esteso
    \item Difficile gestire multiple versioni in produzione
    \item Hotfix = feature branch (nessun trattamento speciale)
\end{itemize}
\end{tcolorbox}
...
\textbf{Adatto per}:
\begin{itemize}
    \item Team senior esperti
    \item CI/CD molto maturo
    \item Test coverage \textgreater 90\%
    \item Deploy multipli al giorno
    \item Culture DevOps forte
\end{itemize}

\textbf{Non adatto per}:
\begin{itemize}
    \item Team junior
    \item Progetti senza CI/CD
    \item Code review richiesta
    \item Release pianificate
\end{itemize}
...
\textbf{Per autore PR}:
\begin{itemize}
    \item PR piccola (max 400 righe, ideale \textless 200)
    \item Descrizione chiara e completa
    \item Self-review prima di request
    \item Test inclusi e passano
    \item Un PR = una feature/fix
    \item Rispondi a feedback costruttivamente
\end{itemize}

\textbf{Per reviewer}:
\begin{itemize}
    \item Review entro 24h (non bloccare altri)
    \item Feedback costruttivo, non critica personale
    \item Distingui: \textit{must fix} vs \textit{nice to have}
    \item Approva quando "abbastanza buono", non "perfetto"
    \item Se molti commenti, call invece di 50 commenti
\end{itemize}
