...
Le librerie standard C forniscono funzioni predefinite per compiti comuni, eliminando la necessità di reimplementare funzionalità già disponibili. Questo capitolo affronta le librerie più utili in dettaglio. La libreria \texttt{string.h} fornisce potenti funzioni per la manipolazione delle stringhe, come il confronto, la copia, la concatenazione e la ricerca. La libreria \texttt{math.h} mette a disposizione operazioni matematiche avanzate, incluse funzioni trigonometriche, logaritmiche, esponenziali e di arrotondamento. La libreria \texttt{time.h} offre strumenti per la gestione del tempo e delle date, permettendo di ottenere l'ora attuale, convertire tra diversi formati temporali e misurare intervalli di tempo. La libreria \texttt{stdlib.h} fornisce funzioni critiche per l'allocazione e la gestione della memoria dinamica, come \texttt{malloc}, \texttt{free}, \texttt{realloc} e \texttt{calloc}. Infine, vedremo funzioni generali per ordinamento e ricerca di dati, come \texttt{qsort} e \texttt{bsearch}, che permettono di implementare algoritmi efficienti.
