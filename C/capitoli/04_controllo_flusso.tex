...
\begin{itemize}
    \item Prendere decisioni (istruzioni condizionali)
    \item Ripetere blocchi di codice (cicli)
    \item Saltare parti di codice
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Incremento}: eseguito dopo ogni iterazione
\end{enumerate}
...
\begin{itemize}
    \item \textbf{for}: quando conosci il numero di iterazioni
    \begin{lstlisting}
    for (int i = 0; i < 10; i++) {
        // Esegui 10 volte
    }
    ...
    \item \textbf{while}: quando la condizione deve essere controllata prima
    \begin{lstlisting}
    while (numero != 0) {
    ...
    \item \textbf{do-while}: quando vuoi eseguire almeno una volta
    \begin{lstlisting}
    do {
