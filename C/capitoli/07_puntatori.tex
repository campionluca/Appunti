...
\begin{itemize}
    \item Manipolazione diretta della memoria
    \item Passaggio efficiente di dati alle funzioni
    \item Allocazione dinamica della memoria
    \item Implementazione di strutture dati complesse
    \item Accesso efficiente agli array
\end{itemize}
...
\begin{description}
    \item[Text Segment:] Contiene il codice eseguibile del programma (istruzioni macchina). Area read-only.
    \item[Data Segment:] Contiene variabili globali e statiche inizializzate esplicitamente nel codice.
    \item[BSS Segment:] Contiene variabili globali e statiche non inizializzate (inizializzate a zero automaticamente).
    \item[Heap:] Area di memoria per allocazione dinamica (malloc, calloc). Cresce verso indirizzi più alti.
    \item[Stack:] Contiene variabili locali, parametri delle funzioni e indirizzi di ritorno. Cresce verso indirizzi più bassi.
\end{description}
...
\begin{itemize}
    \item \texttt{\&} (operatore di indirizzo del linguaggio C): restituisce l'indirizzo di memoria di una variabile
    \item \texttt{*} (operatore di dereferenziazione del linguaggio C): accede al valore contenuto all'indirizzo puntato da un puntatore
\end{itemize}
