...
\begin{itemize}
\item Usa \textbf{nomi al plurale} per collezioni: \texttt{/users}, \texttt{/products}
\item Usa \textbf{kebab-case} per URL multi-parola: \texttt{/order-items}
\item \textbf{Evita verbi} negli URL (usa HTTP methods)
\item Usa \textbf{sostantivi} per identificare risorse
\item Mantieni gerarchia \textbf{massimo 3 livelli}
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
\item \textbf{Discoverability}: Client può esplorare API seguendo link
\item \textbf{Decoupling}: Client meno dipendente da URL hardcoded
\item \textbf{Evolvibilità}: Server può cambiare URL senza rompere client
\item \textbf{Self-documenting}: API più facile da capire
\end{itemize}
...
\begin{itemize}
\item \textbf{SI}: Response > 500 bytes, JSON, XML, testo
\item \textbf{NO}: Immagini (già compresse), response piccole (overhead)
\item \textbf{Algoritmi}: gzip (standard), br (brotli, migliore ratio)
\end{itemize}
\end{tcolorbox}
