...
\textbf{Implicazioni}:
\begin{itemize}
    \item Nessuna sessione server-side
    \item Autenticazione in ogni richiesta (token, API key)
    \item Client mantiene stato applicazione
\end{itemize}
...
\textbf{Vantaggi}:
\begin{itemize}
    \item Scalabilità: qualsiasi server può gestire qualsiasi richiesta
    \item Affidabilità: nessuna sincronizzazione stato tra server
    \item Visibilità: request completa per monitoring/debugging
\end{itemize}

\textbf{Svantaggi}:
\begin{itemize}
    \item Overhead: ripetere dati in ogni richiesta
    \item Performance: nessuna ottimizzazione basata su stato precedente
\end{itemize}
...
\textbf{Vantaggi}:
\begin{itemize}
    \item Scalabilità: aggiungere layer senza modificare client
    \item Security: firewall e gateway nascondono architettura interna
    \item Caching centralizzato
\end{itemize}
...
La maggior parte delle API moderne si ferma al \textbf{Level 2}. Level 3 (HATEOAS) è raro perché:
\begin{itemize}
    \item Complessità implementativa
    \item Client spesso preferiscono URI hardcoded
    \item Overhead payload con link
\end{itemize}
...
\begin{itemize}
    \item Protocollo XML-based
    \item WSDL per definizione interfaccia
    \item WS-Security per sicurezza enterprise
    \item Stateful o stateless
    \item Overhead elevato
\end{itemize}
...
\begin{itemize}
    \item URI resource-based: \texttt{/repos/:owner/:repo}
    \item Versioning in Accept header: \texttt{application/vnd.github.v3+json}
    \item Rate limiting headers: \texttt{X-RateLimit-*}
    \item Hypermedia partial: URL reference in response
    \item Pagination via Link header (RFC 5988)
    \item Authentication: token-based
\end{itemize}
