...
\begin{itemize}
\item \textbf{Status HTTP corretto}: Usa il codice di stato appropriato
\item \textbf{Messaggio chiaro}: Spiega cosa è andato storto
\item \textbf{Codice errore}: Identificatore machine-readable
\item \textbf{Dettagli}: Informazioni aggiuntive per il debugging
\item \textbf{Azioni suggerite}: Come risolvere il problema
\item \textbf{Consistenza}: Stessa struttura per tutti gli errori
\end{itemize}
\end{tcolorbox}
