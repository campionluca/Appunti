...
\begin{itemize}
  \item \textbf{Traffico mobile}: Oltre il 60\% del traffico web proviene da dispositivi mobili
  \item \textbf{SEO}: Google penalizza siti non mobile-friendly
  \item \textbf{User Experience}: Utenti abbandonano siti non ottimizzati per mobile
  \item \textbf{Costi}: Una soluzione invece di due siti separati (desktop + mobile)
  \item \textbf{Manutenzione}: Un solo codebase da aggiornare
\end{itemize}
...
Design che parte da mobile (320px) e scala verso desktop. \textbf{Questo è l'approccio consigliato}.

\textbf{Vantaggi}:
\begin{itemize}
  \item Priorità ai contenuti essenziali
  \item Performance migliori su mobile (meno CSS da sovrascrivere)
  \item Progressive enhancement: aggiungi funzionalità per schermi grandi
  \item SEO-friendly (Google usa mobile-first indexing)
\end{itemize}
