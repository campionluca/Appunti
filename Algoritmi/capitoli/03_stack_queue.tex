...
\textbf{Operazioni fondamentali:} Uno stack supporta cinque operazioni principali. L'operazione \texttt{push(x)} inserisce l'elemento $x$ in cima allo stack. L'operazione \texttt{pop()} rimuove l'elemento in cima e lo restituisce al chiamante. L'operazione \texttt{top()} o \texttt{peek()} permette di ispezionare l'elemento in cima senza rimuoverlo. L'operazione \texttt{isEmpty()} verifica se lo stack non contiene elementi. Infine, l'operazione \texttt{size()} restituisce il numero totale di elementi attualmente presenti nello stack.
...
\textbf{Analisi:} L'implementazione con array ha caratteristiche prestazionali molto favorevoli. Tutte le operazioni fondamentali---push, pop, peek, isEmpty e size---si eseguono in tempo costante $O(1)$. La complessità spaziale è $O(n)$, dove $n$ rappresenta la capacità massima dello stack. Lo svantaggio principale è la necessità di fissare la capacità al momento della creazione, impedendo lo stack di crescere dinamicamente se riceve più elementi del previsto.
...
\textbf{Vantaggi e svantaggi:} L'implementazione con lista concatenata offre due vantaggi importanti: non esiste una capacità massima predefinita e tutte le operazioni rimangono $O(1)$. Tuttavia, presenta anche due svantaggi significativi. Primo, ogni nodo comporta un overhead dovuto all'immagazzinamento dei puntatori, aumentando l'uso di memoria complessivo. Secondo, la peggiore località di cache nella memoria significa che i nodi non occupano posizioni contigue, penalizzando le prestazioni in ambienti moderni dove la cache della CPU è cruciale.
