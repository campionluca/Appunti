...
\begin{itemize}
    \item \textbf{Dimensione fissa}: La dimensione dell'array è determinata alla creazione
    \item \textbf{Accesso diretto}: L'elemento in posizione $i$ è accessibile in tempo $O(1)$
    \item \textbf{Memoria contiguità}: Gli elementi occupano celle consecutive in memoria
    \item \textbf{Tipo omogeneo}: Tutti gli elementi hanno lo stesso tipo
\end{itemize}
...
\textbf{Analisi:}
\begin{itemize}
    \item Caso migliore: $\Theta(1)$ (elemento in prima posizione)
    \item Caso peggiore: $\Theta(n)$ (elemento assente o in ultima posizione)
    \item Caso medio: $\Theta(n)$
\end{itemize}
...
\textbf{Analisi:}
\begin{itemize}
    \item Caso migliore: $\Theta(1)$ (inserimento in coda)
    \item Caso peggiore: $\Theta(n)$ (inserimento in testa, richiede shift di tutti gli elementi)
    \item Caso medio: $\Theta(n)$
\end{itemize}
...
\textbf{Analisi:} Analoga all'inserimento.
\begin{itemize}
    \item Caso migliore: $\Theta(1)$ (cancellazione dell'ultimo elemento)
    \item Caso peggiore: $\Theta(n)$ (cancellazione del primo elemento)
    \item Caso medio: $\Theta(n)$
\end{itemize}
...
\textbf{Strategia di raddoppio:}
\begin{itemize}
    \item Quando l'array è pieno, allochiamo un nuovo array di dimensione doppia
    \item Copiamo tutti gli elementi nel nuovo array
    \item Deallochiamo il vecchio array
\end{itemize}
...
Una \textbf{lista concatenata semplice} è una sequenza di nodi, dove ogni nodo contiene:
\begin{itemize}
    \item Un \textbf{dato} (valore memorizzato)
    \item Un \textbf{puntatore} al nodo successivo
\end{itemize}
Il primo nodo è chiamato \textbf{testa} (head), l'ultimo puntatore è \texttt{NULL}.
\end{definizione}
...
\textbf{Vantaggi:}
\begin{itemize}
    \item Scorrimento bidirezionale
    \item Cancellazione di un nodo dato il puntatore al nodo in $O(1)$
    \item Inserimento prima di un nodo in $O(1)$
\end{itemize}

\textbf{Svantaggi:}
\begin{itemize}
    \item Maggiore uso di memoria (un puntatore in più per nodo)
    \item Maggiore complessità nel mantenere l'invariante dei puntatori
\end{itemize}
...
\textbf{Punti chiave:}
\begin{itemize}
    \item Array: $O(1)$ accesso, $O(n)$ inserimento/cancellazione
    \item Liste: $O(1)$ inserimento/cancellazione in testa, $O(n)$ accesso
    \item Liste doppie: $O(1)$ cancellazione dato il nodo
    \item Array dinamici: $O(1)$ inserimento ammortizzato
\end{itemize}
