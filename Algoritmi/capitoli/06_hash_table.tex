...
Le tabelle hash implementano efficientemente queste operazioni tramite una \textbf{funzione hash} che mappa le chiavi in posizioni di un array.

\section{Concetti fondamentali}

\begin{definizione}[Funzione hash]
Una \textbf{funzione hash} è una funzione $h: U \to \{0, 1, \ldots, m-1\}$ che mappa elementi di un universo $U$ (potenzialmente molto grande) in un insieme di dimensione $m$ (relativamente piccola) di posizioni in un array chiamato \textbf{tabella hash}.
\end{definizione}
...
\begin{enumerate}
    \item Essere \textbf{deterministica}: Stessa chiave → stesso hash
    \item Essere \textbf{veloce} da calcolare: $O(1)$
    \item Distribuire uniformemente le chiavi (minimizzare collisioni)
    \item Minimizzare il clustering (raggruppamento di chiavi)
\end{enumerate}
