...
\begin{itemize}
    \item \textbf{Server-side}: Il codice viene eseguito sul server, non sul client
    \item \textbf{Open Source}: Gratuito, con vasta community e documentazione
    \item \textbf{Cross-platform}: Funziona su Windows, Linux, macOS
    \item \textbf{Database integration}: Supporto nativo per MySQL, PostgreSQL, SQLite, Oracle
    \item \textbf{Sintassi C-like}: Familiare per chi conosce C, Java, JavaScript
    \item \textbf{Loosely typed}: Tipi di variabili determinati automaticamente (PHP < 7.4) o con type hints (PHP 7.4+)
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{float/double}: Numeri decimali
        \item \texttt{string}: Stringhe
        \item \texttt{bool}: true/false
    \end{itemize}
    \item \textbf{Composti}:
    \begin{itemize}
        \item \texttt{array}: Array
        \item \texttt{object}: Oggetti
    \end{itemize}
    \item \textbf{Speciali}:
    \begin{itemize}
        \item \texttt{NULL}: Valore nullo
        \item \texttt{resource}: Riferimenti risorse esterne
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{E\_ERROR}: Errori fatali (script termina)
    \item \textbf{E\_WARNING}: Warning (script continua)
    \item \textbf{E\_NOTICE}: Notice (possibili problemi)
    \item \textbf{E\_PARSE}: Errori di sintassi
    \item \textbf{E\_ALL}: Tutti gli errori
\end{itemize}
...
\begin{itemize}
    \item PHP è un linguaggio server-side per web development
    \item Richiede stack LAMP/WAMP con Apache, MySQL, PHP
    \item Script PHP embedded in HTML con \texttt{<?php ... ?>}
    \item Superglobali (\texttt{\$\_GET}, \texttt{\$\_POST}, \texttt{\$\_SERVER}) forniscono accesso a richieste HTTP
    \item Type juggling automatico ma attenzione ai bug
    \item Gestione errori essenziale (development vs production)
\end{itemize}
