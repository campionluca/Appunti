...
I file di testo sono fondamentali per:
\begin{itemize}
  \item \textbf{Persistenza dati}: configurazioni, log, cache, sessioni
  \item \textbf{Interoperabilità}: CSV, JSON, XML per scambio dati
  \item \textbf{Debugging}: log applicativi, trace, dump
  \item \textbf{Import/Export}: dati utente, backup, migrazioni
  \item \textbf{Comunicazione}: file come IPC (Inter-Process Communication)
\end{itemize}

\subsection{Operazioni di alto vs basso livello}
PHP offre due approcci:

\textbf{Alto livello} (funzioni pratiche):
\begin{itemize}
  \item \texttt{file\_get\_contents()}, \texttt{file\_put\_contents()}
  \item \texttt{file()}, \texttt{readfile()}
  \item Caricano tutto in memoria: semplici ma non scalabili per file grandi
\end{itemize}

\textbf{Basso livello} (controllo granulare):
\begin{itemize}
  \item \texttt{fopen()}, \texttt{fread()}, \texttt{fwrite()}, \texttt{fclose()}
  \item \texttt{fgets()}, \texttt{fgetcsv()}, \texttt{flock()}
  \item Leggono a blocchi: più complessi ma efficienti con file di GB
\end{itemize}
...
\begin{lstlisting}
<?php
// LOCK_SH = Shared lock (lettura): altri possono leggere, nessuno scrive
// LOCK_EX = Exclusive lock (scrittura): nessun altro accesso
// LOCK_UN = Unlock: rilascia il lock
// LOCK_NB = Non-blocking: non aspetta, fallisce subito se già locked
?>
\end{lstlisting}
