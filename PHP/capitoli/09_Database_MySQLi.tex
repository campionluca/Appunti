...
\subsection{Esempio: Modello ER di un Blog}

Prima di implementare query, è fondamentale progettare la struttura del database. Ecco un esempio di diagramma ER (Entity-Relationship) per un semplice sistema di blog:

\begin{center}
\begin{tikzpicture}[
    entity/.style={rectangle, draw, thick, minimum width=3cm, minimum height=0.8cm, fill=blue!20, font=\small\bfseries},
    attribute/.style={ellipse, draw, minimum width=1.8cm, minimum height=0.6cm, fill=yellow!20, font=\footnotesize},
    relationship/.style={diamond, draw, thick, minimum width=2cm, minimum height=1cm, fill=green!20, font=\small},
    pk/.style={attribute, fill=orange!30},
    fk/.style={attribute, fill=red!20}
]
    % Entities
    \node[entity] (users) at (0,0) {USERS};
    \node[entity] (posts) at (6,0) {POSTS};
    \node[entity] (comments) at (6,-4) {COMMENTS};

    % USERS attributes
    \node[pk] (user_id) at (-2.5,1.5) {\underline{id}};
    \node[attribute] (user_name) at (0,2) {username};
    \node[attribute] (user_email) at (2.5,1.5) {email};
    \node[attribute] (user_pass) at (-2.5,-1.5) {password\_hash};
    \node[attribute] (user_created) at (2.5,-1.5) {created\_at};

    % POSTS attributes
    \node[pk] (post_id) at (8.5,1.5) {\underline{id}};
    \node[attribute] (post_title) at (6,2.5) {title};
    \node[attribute] (post_content) at (3.5,1) {content};
    \node[attribute] (post_created) at (8.5,-1) {created\_at};
    \node[fk] (post_user_fk) at (3.5,-0.5) {user\_id (FK)};

    % COMMENTS attributes
    \node[pk] (comment_id) at (8.5,-2.5) {\underline{id}};
    \node[attribute] (comment_content) at (6,-2) {content};
    \node[attribute] (comment_created) at (8.5,-5) {created\_at};
    \node[fk] (comment_user_fk) at (3.5,-3) {user\_id (FK)};
    \node[fk] (comment_post_fk) at (3.5,-4.5) {post\_id (FK)};

    % Relationships
    \node[relationship] (writes_posts) at (3,0) {scrive};
    \node[relationship] (writes_comments) at (3,-2.5) {scrive};
    \node[relationship] (has_comments) at (6,-2) {ha};

    % Connections - Users to Posts
    \draw[thick] (users) -- (writes_posts);
    \draw[thick] (writes_posts) -- (posts);
    \node[above, font=\tiny] at (1.5,0.3) {1};
    \node[above, font=\tiny] at (4.5,0.3) {N};

    % Connections - Users to Comments
    \draw[thick] (users) |- (writes_comments);
    \draw[thick] (writes_comments) -- (comments);
    \node[left, font=\tiny] at (0.3,-2.2) {1};
    \node[above, font=\tiny] at (4.5,-2.2) {N};

    % Connections - Posts to Comments
    \draw[thick] (posts) -- (has_comments);
    \draw[thick] (has_comments) -- (comments);
    \node[right, font=\tiny] at (5.7,-0.8) {1};
    \node[right, font=\tiny] at (5.7,-3.2) {N};

    % Attribute connections
    \draw (users) -- (user_id);
    \draw (users) -- (user_name);
    \draw (users) -- (user_email);
    \draw (users) -- (user_pass);
    \draw (users) -- (user_created);

    \draw (posts) -- (post_id);
    \draw (posts) -- (post_title);
    \draw (posts) -- (post_content);
    \draw (posts) -- (post_created);
    \draw (posts) -- (post_user_fk);

    \draw (comments) -- (comment_id);
    \draw (comments) -- (comment_content);
    \draw (comments) -- (comment_created);
    \draw (comments) -- (comment_user_fk);
    \draw (comments) -- (comment_post_fk);
\end{tikzpicture}
\end{center}

\begin{tcolorbox}[colback=green!10, colframe=green!60, title=Spiegazione Modello ER]
\textbf{Entità}:
\begin{itemize}
    \item \textbf{USERS}: Utenti del blog (autori e commentatori)
    \item \textbf{POSTS}: Articoli pubblicati dagli utenti
    \item \textbf{COMMENTS}: Commenti agli articoli
\end{itemize}

\textbf{Relazioni}:
\begin{itemize}
    \item \textbf{USERS scrive POSTS}: 1:N (un utente può scrivere molti post)
    \item \textbf{USERS scrive COMMENTS}: 1:N (un utente può scrivere molti commenti)
    \item \textbf{POSTS ha COMMENTS}: 1:N (un post può avere molti commenti)
\end{itemize}

\textbf{Attributi}:
\begin{itemize}
    \item \textcolor{orange}{\textbf{PK (Primary Key)}}: Chiave primaria sottolineata (es. \underline{id})
    \item \textcolor{red}{\textbf{FK (Foreign Key)}}: Chiave esterna che referenzia altra tabella
    \item Ogni entità ha \texttt{created\_at} per timestamp creazione
\end{itemize}
\end{tcolorbox}

\subsection{Schema SQL corrispondente}

\begin{lstlisting}[language=SQL, caption={Schema database blog}]
CREATE TABLE users (
...
\begin{itemize}
    \item \texttt{ON DELETE CASCADE}: Se elimini un utente, elimina anche i suoi post e commenti
    \item \texttt{ON DELETE SET NULL}: Alternativa che imposta FK a NULL invece di eliminare
    \item Usa \texttt{ENGINE=InnoDB} per supportare foreign keys (MyISAM non le supporta)
    \item Sempre indicizzare colonne FK per performance JOIN
\end{itemize}
\end{attenzione}
...
\subsection{Stati di una Transazione}

Una transazione attraversa diversi stati durante il suo ciclo di vita:

\begin{center}
\begin{tikzpicture}[
    node distance=2.5cm,
    state/.style={rectangle, draw, thick, minimum width=2.5cm, minimum height=1cm, align=center, font=\small\bfseries, rounded corners},
    active/.style={state, fill=blue!20},
    partial/.style={state, fill=yellow!30},
    success/.style={state, fill=green!30},
    fail/.style={state, fill=red!20},
    abort/.style={state, fill=gray!30},
    arrow/.style={->, thick, >=stealth}
]
    % Stati
    \node[active] (active) at (0,0) {ACTIVE\\(Esecuzione)};
    \node[partial] (partial) at (5,0) {PARTIALLY\\COMMITTED};
    \node[success] (committed) at (5,-3) {COMMITTED\\(Successo)};
    \node[fail] (failed) at (0,-3) {FAILED\\(Errore)};
    \node[abort] (aborted) at (-2.5,-1.5) {ABORTED\\(Rollback)};

    % Transizioni
    \draw[arrow, blue] (-3,0) -- node[above, font=\footnotesize] {BEGIN TRANSACTION} (active);

    \draw[arrow, blue] (active) -- node[above, font=\footnotesize] {Tutte le operazioni\\eseguite} (partial);

    \draw[arrow, green!60!black, very thick] (partial) -- node[right, font=\footnotesize] {COMMIT\\(scrittura su disco)} (committed);

    \draw[arrow, red] (active) -- node[left, font=\footnotesize] {Errore durante\\esecuzione} (failed);

    \draw[arrow, red] (partial) -| node[near end, right, font=\footnotesize] {Fallimento\\commit} (failed);

    \draw[arrow, red] (failed) -- node[above, font=\footnotesize] {ROLLBACK\\(annulla modifiche)} (aborted);

    % Terminazione
    \draw[arrow, green!60!black] (committed) -- node[below, font=\footnotesize] {Transazione completata} (8,-3);
    \draw[arrow, gray] (aborted) -- node[below, font=\footnotesize] {Transazione annullata} (-5.5,-1.5);

    % Legenda
    \node[below, font=\footnotesize, align=left] at (2.5,-4.5) {
        \textcolor{blue}{$\rightarrow$ Flusso normale} \quad
        \textcolor{red}{$\rightarrow$ Gestione errori} \quad
        \textcolor{green!60!black}{$\rightarrow$ Successo}
    };
\end{tikzpicture}
\end{center}

\begin{tcolorbox}[colback=blue!10, colframe=blue!60, title=Spiegazione stati transazione]
\begin{itemize}
    \item \textbf{ACTIVE}: Transazione in esecuzione dopo \texttt{BEGIN TRANSACTION}. Le operazioni vengono eseguite ma non sono ancora permanenti.

    \item \textbf{PARTIALLY COMMITTED}: Tutte le operazioni sono state eseguite con successo, ma le modifiche non sono ancora scritte permanentemente su disco.

    \item \textbf{COMMITTED}: Il \texttt{COMMIT} è stato eseguito e tutte le modifiche sono permanenti su disco (ACID Durability). La transazione è completata con successo.

    \item \textbf{FAILED}: Si è verificato un errore durante l'esecuzione o il commit. Le modifiche sono ancora in memoria/log ma non permanenti.

    \item \textbf{ABORTED}: Il \texttt{ROLLBACK} ha annullato tutte le modifiche. Il database torna allo stato precedente alla transazione.
\end{itemize}
\end{tcolorbox}

\begin{attenzione}
In PHP MySQLi, il passaggio da PARTIALLY COMMITTED a COMMITTED avviene automaticamente quando chiami \texttt{\$mysqli->commit()}. Se si verifica un errore prima del commit, devi chiamare esplicitamente \texttt{\$mysqli->rollback()} per passare allo stato ABORTED.
\end{attenzione}
