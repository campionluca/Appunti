...
\begin{lstlisting}[language=docker, caption={Dockerfile NON Ottimizzato}]
# ❌ BAD: Inefficient, large image, security issues
FROM node:20

WORKDIR /app

# ❌ Copia tutto (inclusi node_modules, .git, etc)
COPY . .
...
\begin{lstlisting}[language=docker, caption={Dockerfile OTTIMIZZATO}]
...
\begin{lstlisting}[language=docker, caption={Optimal Layer Order}]
...
\begin{lstlisting}[language=docker, caption={BuildKit Cache Mounts e Secrets}]
# syntax=docker/dockerfile:1.4
...
\begin{lstlisting}[language=docker, caption={Multiple User Strategies}]
...
\begin{lstlisting}[language=yaml, caption={Read-Only Filesystem in Docker Compose}]
version: '3.8'
...
\begin{lstlisting}[language=yaml, caption={Network Segmentation}]
...
\begin{lstlisting}[language=yaml, caption={NetworkPolicy - Deny All by Default}]
# deny-all.yaml
...
\begin{lstlisting}[language=yaml, caption={Resource Limits in Docker Compose}]
version: '3.8'
...
\begin{lstlisting}[language=yaml, caption={HashiCorp Vault Integration}]
