...
\begin{lstlisting}[, caption=docker-compose.yml minimale]
version: '3.8'
...
\begin{lstlisting}[, caption=Service con build custom]
services:
  app:
    build:
      context: ./app
...
\begin{lstlisting}[, caption=Service con immagine da registry]
services:
  redis:
...
\begin{lstlisting}[, caption=Definire dipendenze]
services:
  web:
    image: myapp
    depends_on:
...
\begin{lstlisting}[, caption=Aspettare che DB sia pronto]
services:
  db:
    image: postgres:15
    environment:
...
\begin{lstlisting}[, caption=Definire reti multiple]
services:
  frontend:
    image: react-app
    networks:
...
\begin{lstlisting}[, caption=Volumi gestiti da Docker]
services:
  db:
    image: postgres:15
    volumes:
...
\begin{lstlisting}[, caption=Mount directory host]
services:
  web:
    image: nginx
    volumes:
...
\begin{lstlisting}[, caption=Usare variabili da .env]
services:
  db:
...
\begin{lstlisting}[, caption=docker-compose.yml per LAMP]
version: '3.8'
...
\begin{lstlisting}[, caption=docker-compose.yml per MERN]
version: '3.8'
...
\begin{lstlisting}[, caption=Architettura microservizi]
version: '3.8'
...
\begin{lstlisting}[, caption=Stack monitoring]
version: '3.8'
...
\begin{lstlisting}[, caption=docker-compose.override.yml]
