...
\begin{itemize}
    \item No automatic service discovery (DNS)
    \item Comunicazione solo via IP
    \item Tutti i container vedono tutti gli altri
\end{itemize}
...
\textbf{Vantaggi}:
\begin{itemize}
    \item Performance ottimale (no NAT overhead)
    \item Accesso diretto a tutte le interfacce host
\end{itemize}

\textbf{Svantaggi}:
\begin{itemize}
    \item Nessun isolamento di rete
    \item Port conflicts se più container usano stessa porta
    \item Non funziona su Docker Desktop (macOS/Windows)
\end{itemize}

\textbf{Quando usarlo}:
\begin{itemize}
    \item Performance critiche (monitoring, load balancer)
    \item Container che gestisce tutto il network stack (VPN, firewall)
\end{itemize}
