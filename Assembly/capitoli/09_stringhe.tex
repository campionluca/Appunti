...
\begin{itemize}
    \item \textbf{SI}: Source Index (DS:SI per default)
    \item \textbf{DI}: Destination Index (ES:DI sempre)
    \item \textbf{CX}: Contatore per prefissi REP
    \item \textbf{DF}: Direction Flag (0=incrementa, 1=decrementa)
\end{itemize}
...
\begin{lstlisting}
REPE CMPSB         ; Ripete finche' uguale e CX!=0
\end{lstlisting}

\subsection{REPNE/REPNZ}

\begin{lstlisting}
REPNE SCASB        ; Ripete finche' diverso e CX!=0
\end{lstlisting}
...
strcmp PROC
    ...
    REPE CMPSB         ; Confronta finche' uguali
    JE equal
    ; Diverse
    RET
