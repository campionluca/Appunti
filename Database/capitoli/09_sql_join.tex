...
\subsection{Panoramica Tipi di JOIN}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=0.7]
        ...
        \begin{scope}[xshift=0cm]
            ...
            \draw[thick] (1.8,0) circle (1.2);
            \node at (-0.8, 0.5) {\footnotesize A};
            \node at (2.6, 0.5) {\footnotesize B};

            % Intersezione
            \begin{scope}
                \clip (0,0) circle (1.2);
                \fill[blue!40] (1.8,0) circle (1.2);
            \end{scope}

            \node[below, font=\small\bfseries] at (0.9, -1.8) {INNER JOIN};
            \node[below, font=\footnotesize, align=center] at (0.9, -2.2) {Solo righe\\con match};
        \end{scope}
        ...
        \begin{scope}[xshift=5.5cm]
            ...
            % Tutto A
            \fill[green!40] (0,0) circle (1.2);

            \node[below, font=\small\bfseries] at (0.9, -1.8) {LEFT JOIN};
            \node[below, font=\footnotesize, align=center] at (0.9, -2.2) {Tutte A\\+ match B};
        \end{scope}
        ...
        \begin{scope}[xshift=11cm]
            ...
            % Tutto B
            \fill[orange!40] (1.8,0) circle (1.2);

            \node[below, font=\small\bfseries] at (0.9, -1.8) {RIGHT JOIN};
            \node[below, font=\footnotesize, align=center] at (0.9, -2.2) {Tutte B\\+ match A};
        \end{scope}
        ...
        \begin{scope}[xshift=0cm, yshift=-6cm]
            ...
            % Entrambi
            \fill[purple!30] (0,0) circle (1.2);
            \fill[purple!30] (1.8,0) circle (1.2);

            \node[below, font=\small\bfseries] at (0.9, -1.8) {FULL OUTER JOIN};
            \node[below, font=\footnotesize, align=center] at (0.9, -2.2) {Tutte A + Tutte B\\(MySQL: usa UNION)};
        \end{scope}
        ...
        \begin{scope}[xshift=5.5cm, yshift=-6cm]
            ...
            \foreach \x in {0,0.83,1.66} {
                \foreach \y in {0,0.66,1.33} {
                    \fill[red!20] (\x,\y) rectangle (\x+0.7,\y+0.6);
                    \draw (\x,\y) rectangle (\x+0.7,\y+0.6);
                }
            }
            \node at (-0.3, 1) {\footnotesize A};
            \node at (1.25, 2.3) {\footnotesize B};

            \node[below, font=\small\bfseries] at (1.25, -1.8) {CROSS JOIN};
            \node[below, font=\footnotesize, align=center] at (1.25, -2.2) {Prodotto\\cartesiano};
        \end{scope}
        ...
        \begin{scope}[xshift=11cm, yshift=-6cm]
            ...
            % Solo parte sinistra senza intersezione
            \fill[yellow!50] (0,0) circle (1.2);
            \begin{scope}
                \clip (0,0) circle (1.2);
                \fill[white] (1.8,0) circle (1.2);
            \end{scope}

            \node[below, font=\small\bfseries, align=center] at (0.9, -1.8) {LEFT EXCLUDING};
            \node[below, font=\footnotesize, align=center] at (0.9, -2.2) {Solo A\\senza match};
        \end{scope}
    \end{tikzpicture}
    \caption{Tipi di JOIN: confronto visuale}
\end{figure}

\begin{tcolorbox}[colback=blue!10, colframe=blue!60, title=Quale JOIN usare?]
\begin{itemize}
    \item \textbf{INNER JOIN}: quando vuoi solo le righe che hanno corrispondenza in entrambe le tabelle
    \item \textbf{LEFT JOIN}: quando vuoi tutte le righe della tabella principale (anche senza match)
    \item \textbf{RIGHT JOIN}: come LEFT JOIN ma al contrario (raramente usato, preferisci LEFT)
    \item \textbf{FULL OUTER JOIN}: quando vuoi tutte le righe di entrambe (raro, MySQL non supporta)
    \item \textbf{CROSS JOIN}: quando vuoi tutte le combinazioni possibili (attenzione: risultati enormi!)
    \item \textbf{LEFT EXCLUDING}: usa \texttt{LEFT JOIN + WHERE destra.chiave IS NULL} per trovare righe senza match
\end{itemize}
\end{tcolorbox}
