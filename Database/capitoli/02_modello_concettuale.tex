...
\begin{description}
    \item[\textbf{Semplice}] Non è scomponibile (es. \texttt{email})
    \item[\textbf{Composto}] Scomponibile in attributi più piccoli (es. \texttt{indirizzo} = \texttt{via, numero, città, CAP})
    \item[\textbf{Univoco/Chiave}] Identifica univocamente l'entità (es. \texttt{idCliente})
    \item[\textbf{Opzionale}] Può non avere un valore (es. \texttt{numeroCellulare})
    \item[\textbf{Multivalore}] Può assumere più valori (es. \texttt{telefoni} di una ditta)
\end{description}
...
\begin{description}
    \item[\textbf{1:1 (uno a uno)}] Ogni elemento di A è associato a esattamente un elemento di B e viceversa.
    \item[\textbf{1:N (uno a molti)}] Ogni elemento di A è associato a uno o più elementi di B, ma ogni elemento di B è associato a un solo elemento di A.
    \item[\textbf{N:M (molti a molti)}] Ogni elemento di A può essere associato a più elementi di B e viceversa.
    \item[\textbf{0..1}] Opzionale: da zero a uno
    \item[\textbf{1..N}] Obbligatorio: da uno a molti
\end{description}
...
\begin{itemize}
    \item Un'\textbf{entità} rappresenta una classe di oggetti del dominio
    \item Un \textbf{attributo} descrive una proprietà dell'entità
    \item Una \textbf{relazione} collega due o più entità
    \item La \textbf{cardinalità} specifica il numero di associazioni (1:1, 1:N, N:M)
    \item La \textbf{chiave primaria} identifica univocamente l'entità
    \item La \textbf{chiave esterna} crea il collegamento tra entità
\end{itemize}
\end{tcolorbox}
