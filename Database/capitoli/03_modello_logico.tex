...
Il modello relazionale si fonda su tre concetti strutturali essenziali. Una \textbf{relazione} (o tabella) è una collezione organizzata di tuple (righe) che condividono lo stesso insieme di attributi (colonne), formando una struttura bidimensionale coerente. Un \textbf{attributo} (o colonna) rappresenta una proprietà della relazione, caratterizzato da un nome distintivo e un dominio che specifica il tipo di dato che può contenere. Una \textbf{tupla} (o riga) è un record che contiene un valore specifico per ogni attributo della relazione, rappresentando una singola istanza dell'entità modellata.
...
Una \textbf{chiave primaria} è un attributo o insieme di attributi con caratteristiche fondamentali per l'integrità del database. Innanzitutto, identifica univocamente ogni tupla della relazione, assicurando che non ci siano due record identici. In secondo luogo, non può contenere valori NULL, garantendo che ogni record sia sempre identificabile. Infine, non può avere duplicati, rafforzando il vincolo di unicità che è essenziale per il funzionamento corretto del sistema.
...
Una \textbf{relazione} nel modello logico rappresenta una tabella strutturata con attributi (colonne) e tuple (righe). La \textbf{chiave primaria} riveste un ruolo cruciale identificando univocamente ogni tupla e garantendo l'integrità dell'entità. Una \textbf{chiave esterna} collega tabelle diverse mantenendo l'integrità referenziale, assicurando che i riferimenti rimangono coerenti. I \textbf{vincoli di integrità} sono regole che garantiscono la coerenza e la validità dei dati in tutte le operazioni. Le relazioni N:M, caratterizzate da una complessità maggiore, richiedono la creazione di una \textbf{tabella di giunzione} che decomppone la relazione complessa in due relazioni più semplici. Ogni diagramma ER viene tradotto sistematicamente e metodicamente in un insieme di tabelle relazionali, preservando la semantica e le relazioni del modello concettuale.
\end{tcolorbox}
