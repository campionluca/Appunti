...
\textbf{Caratteristiche innovative di Unix:}

\begin{itemize}
    \item Sistema multitasking e multiutente
    \item Filesystem gerarchico
    \item File di testo per configurazione
    \item Shell come interfaccia utente programmabile
    \item Pipe per concatenare comandi
    \item Portabilità (dopo la riscrittura in C nel 1973)
\end{itemize}
...
\subsection{Tipi di Shell}

\begin{itemize}
    \item \textbf{sh} (Bourne Shell): shell originale Unix
    \item \textbf{bash} (Bourne Again Shell): shell standard su Linux
    \item \textbf{zsh} (Z Shell): estensioni avanzate, popolare su macOS
    \item \textbf{fish} (Friendly Interactive Shell): syntax highlighting, autosuggestions
    \item \textbf{dash} (Debian Almquist Shell): leggera, usata per script di sistema
\end{itemize}
...
\begin{itemize}
    \item La storia di Unix e Linux: dalle origini ai giorni nostri
    \item L'architettura a livelli del sistema Linux
    \item Il ruolo del kernel e le sue responsabilità
    \item Le principali famiglie di distribuzioni e come scegliere
    \item La shell Bash: prompt, history, job control
    \item Variabili d'ambiente e PATH
    \item File di configurazione e personalizzazione
\end{itemize}
