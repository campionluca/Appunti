...
\begin{itemize}
    \item \texttt{cp} sovrascrive silenziosamente i file esistenti (usa \texttt{-i} per conferma)
    \item Per directory, \texttt{-r} o \texttt{-R} è obbligatorio
    \item \texttt{cp dir1 dir2} si comporta diversamente se dir2 esiste o no
    \item Se dir2 esiste: crea dir2/dir1
    \item Se dir2 non esiste: crea dir2 con contenuto di dir1
\end{itemize}
\end{tcolorbox}
...
\textbf{Il comando \texttt{rm -rf} è estremamente pericoloso!}

\begin{itemize}
    \item Non chiede conferma (\texttt{-f})
    \item Cancella ricorsivamente (\texttt{-r})
    \item Non c'è cestino: i file sono persi per sempre
    \item Fate SEMPRE doppio check prima di eseguire
    \item Mai eseguire: \texttt{rm -rf /} o \texttt{rm -rf /*}
    \item Considerate alias: \texttt{alias rm='rm -i'}
\end{itemize}
