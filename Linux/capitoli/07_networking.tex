...
\begin{itemize}
    \item Configurare e visualizzare interfacce di rete con \texttt{ip} e \texttt{ifconfig}.
    \item Diagnosticare problemi di rete con \texttt{netstat}, \texttt{ss}, \texttt{ping}, \texttt{traceroute}.
    \item Gestire connessioni remote sicure con SSH.
    \item Trasferire file con \texttt{scp}, \texttt{rsync}.
    \item Scaricare e interagire con API tramite \texttt{curl} e \texttt{wget}.
\end{itemize}
...
\item Diagnosticare problemi con \texttt{ping}, \texttt{traceroute}, \texttt{ss}, \texttt{netstat}
\item Gestire connessioni remote sicure con SSH e tunneling
\item Trasferire file efficientemente con \texttt{scp} e \texttt{rsync}
\item Interagire con API e scaricare contenuti con \texttt{curl} e \texttt{wget}
\item Implementare monitoring e automazione delle operazioni di rete
\end{itemize}
